...
| 후보 | 합리적인가 | 이유 |
| ---- | ---------- | ---- |
| `root` | 𝐗 | 모니터링은 root 권한이 필요 없다. root 로 도는 cron 작업이 많을수록 사고 반경 ↑ |
| `agent-dev` | ▲ | dev 는 "작성자". cron 처럼 **계속 도는 자동 프로세스의 소유자** 는 운영자(`agent-admin`) 인 게 자연스럽다 |
| `agent-test` | 𝐗 | test 는 `agent-core` 그룹이 아니라 `/var/log/agent-app` 에 쓸 권한이 없다 |
| **`agent-admin`** | ✔️ | 운영자이면서 `agent-core` 멤버라 로그 디렉토리 쓰기 가능. monitor.sh 실행 권한도 그룹(`agent-core`)으로 확보 |
...
> agent-test는 테스트 업무만 수행하면 되므로 API 키와 운영 로그에 접근할 필요가 없고, 불필요한 접근 권한을 주는 것 자체가 보안 위협이 되기 때문에
...
| 표현 | 원래 의미 | 설명 |
| :--- | :--- | :--- |
| `LOG_FILE` | Log File | 모니터링 결과를 누적 저장할 로그 파일 경로입니다. |
| `MAX_LOG_SIZE` | Maximum Log Size | 로그 파일 최대 크기입니다. 스크립트에서는 `10 * 1024 * 1024`로 10MB를 의미합니다. |
| `MAX_LOG_COUNT` | Maximum Log Count | 순환 보관할 로그 파일 최대 개수입니다. 스크립트에서는 최대 10개를 유지합니다. |
| `APP_NAMES` | Application Names | 확인할 애플리케이션 프로세스 이름 목록입니다. |
| `APP_PORT` | Application Port | 앱이 LISTEN해야 하는 포트 번호입니다. 스크립트에서는 `15034`입니다. |
| `PID` | Process ID | 실행 중인 프로세스의 고유 번호입니다. |
| `RUNNING_APP` | Running Application | 실제로 실행 중이라고 확인된 앱 이름입니다. |
| `APP_NAME` | Application Name | `APP_NAMES` 배열을 반복하면서 하나씩 꺼낸 앱 이름입니다. |
| `PORT_CHECK` | Port Check | 지정 포트가 LISTEN 중인지 확인한 결과입니다. |
| `UFW_STATUS` | UFW Status | UFW 방화벽 활성화 여부를 확인한 결과입니다. |
| `CPU` | Central Processing Unit | CPU 사용률 값을 저장합니다. |
| `MEM` | Memory | 메모리 사용률 값을 저장합니다. |
| `DISK` | Disk | 디스크 사용률 값을 저장합니다. 여기서는 루트 파티션 `/` 기준입니다. |
| `CPU_INT` | CPU Integer | CPU 사용률에서 소수점 이하를 제거한 정수 값입니다. Bash 숫자 비교용입니다. |
| `MEM_INT` | Memory Integer | 메모리 사용률에서 소수점 이하를 제거한 정수 값입니다. Bash 숫자 비교용입니다. |
| `TIMESTAMP` | Time Stamp | 로그에 남길 현재 시각입니다. |
| `LOG_SIZE` | Log Size | 현재 로그 파일 크기입니다. `stat` 명령으로 바이트 단위 크기를 가져옵니다. |
...
| 명령어 | 원래 의미/어원 | 설명 |
| :--- | :--- | :--- |
| `cat` | concatenate | 파일 내용을 출력하거나, 여러 줄 내용을 파일로 저장할 때 사용합니다. |
| `bash` | Bourne Again SHell | Bash 셸입니다. `#!/bin/bash`는 이 스크립트를 Bash로 실행하라는 의미입니다. |
| `echo` | echo | 문자열을 화면에 출력합니다. 상태 메시지와 로그 라인 출력에 사용합니다. |
| `pgrep` | process grep | 실행 중인 프로세스 중 특정 이름과 일치하는 프로세스의 PID를 찾습니다. |
| `ss` | socket statistics | 현재 열려 있는 소켓과 포트 상태를 확인합니다. |
| `grep` | global regular expression print | 특정 문자열이 포함된 줄을 찾습니다. |
| `top` | table of processes | CPU 사용률과 프로세스 상태를 확인합니다. |
| `awk` | Aho, Weinberger, Kernighan | 텍스트에서 특정 필드를 뽑거나 계산할 때 사용하는 도구입니다. 이름은 만든 사람들의 성에서 왔습니다. |
| `cut` | cut | 문자열을 구분자로 잘라 특정 필드만 가져옵니다. |
| `free` | free memory | 메모리 사용량을 확인합니다. |
| `df` | disk free | 디스크 사용량과 여유 공간을 확인합니다. |
| `tail` | tail | 파일이나 명령 출력의 마지막 부분을 보여줍니다. |
| `date` | date | 현재 날짜와 시간을 출력합니다. |
| `stat` | status | 파일의 상태 정보를 확인합니다. 스크립트에서는 파일 크기 확인에 사용합니다. |
| `seq` | sequence | 숫자 순서를 생성합니다. 로그 파일 번호를 뒤에서 앞으로 이동할 때 사용합니다. |
| `mv` | move | 파일을 이동하거나 이름을 변경합니다. 로그 순환에 사용합니다. |
| `touch` | touch | 빈 파일을 만들거나 파일의 시간을 갱신합니다. |
| `chown` | change owner | 파일 소유자와 그룹을 변경합니다. |
| `chmod` | change mode | 파일 권한을 변경합니다. |
...
| 표현 | 원래 의미 | 설명 |
| :--- | :--- | :--- |
| `pgrep` | process grep | 프로세스를 검색해서 PID를 출력합니다. |
| `-f` | full | 프로세스 이름만이 아니라 전체 실행 명령줄(full command line)에서 검색합니다. |
...
`-f` 옵션이 있어야 전체 명령줄 안의 `agent_app.py`까지 찾기 좋습니다.
...
| `-t` | TCP | TCP 소켓을 보여줍니다. |
| `-u` | UDP | UDP 소켓을 보여줍니다. |
| `-l` | listening | LISTEN 상태인 소켓을 보여줍니다. |
| `-n` | numeric | 서비스 이름이 아니라 숫자 포트로 보여줍니다. 예: `15034` |
| `-p` | process | 해당 포트를 사용하는 프로세스 정보를 보여줍니다. |

즉, 아래 명령은 “TCP/UDP 중 LISTEN 중인 포트를 숫자로 보여주고, 어떤 프로세스가 쓰는지도 보여줘”라는 뜻입니다.
...
| 옵션 | 원래 의미 | 설명 |
| :--- | :--- | :--- |
| `-b` | batch | 배치 모드입니다. 사람이 보는 실시간 화면이 아니라 스크립트가 처리하기 쉬운 출력으로 보여줍니다. |
| `-n1` | number 1 | `top`을 1번만 실행합니다. |

즉, 아래 명령은 “top을 스크립트용 출력으로 한 번만 실행해줘”라는 의미입니다.
...
| 표현 | 의미 | 설명 |
| :--- | :--- | :--- |
| `awk` | 텍스트 필드 처리 도구 | 공백 기준으로 나뉜 필드에서 원하는 값을 뽑습니다. |
| `print` | 출력 | 지정한 값을 출력합니다. |
| `$2` | 두 번째 필드 | 한 줄을 공백 기준으로 나눴을 때 두 번째 값입니다. |

---

### 3.5 `awk '{printf "%.1f", $3/$2 * 100}'`

| 표현 | 의미 | 설명 |
| :--- | :--- | :--- |
| `printf` | formatted print | 형식을 지정해서 출력합니다. |
| `"%.1f"` | 소수점 1자리 실수 | 예: `4.3`, `10.0`처럼 소수점 한 자리로 출력합니다. |
| `$3` | 세 번째 필드 | 사용 중인 메모리 값입니다. |
| `$2` | 두 번째 필드 | 전체 메모리 값입니다. |
| `$3/$2 * 100` | 사용률 계산 | 사용 중 메모리 / 전체 메모리 * 100 계산입니다. |
...
| 표현 | 의미 | 설명 |
| :--- | :--- | :--- |
| `tail` | 마지막 부분 출력 | 출력의 마지막 줄을 가져옵니다. |
| `-1` | 마지막 1줄 | 마지막 한 줄만 출력합니다. |

`df /`는 보통 헤더 1줄과 실제 데이터 1줄을 출력하므로, `tail -1`로 실제 데이터 줄만 가져옵니다.
...
| 표현 | 원래 의미 | 설명 |
| :--- | :--- | :--- |
| `stat` | status | 파일 상태 정보를 확인합니다. |
| `-c` | format | 출력 형식을 지정합니다. |
| `%s` | size | 파일 크기를 바이트 단위로 출력합니다. |

즉, 아래 명령은 “로그 파일 크기를 바이트 단위로 출력해줘”라는 뜻입니다.
...
### 3.9 `seq $((MAX_LOG_COUNT-1)) -1 1`

| 표현 | 의미 | 설명 |
| :--- | :--- | :--- |
| `seq` | sequence | 숫자 범위를 생성합니다. |
| 첫 번째 값 | 시작값 | 여기서는 `MAX_LOG_COUNT-1`, 즉 9입니다. |
| 두 번째 값 `-1` | 증가/감소 간격 | 1씩 감소합니다. |
| 세 번째 값 `1` | 끝값 | 1까지 반복합니다. |

`MAX_LOG_COUNT=10`이면 실제로는 아래 순서가 만들어집니다.

```text
9 8 7 6 5 4 3 2 1
```

뒤에서 앞으로 처리하는 이유는 기존 로그 파일을 덮어쓰지 않기 위해서입니다.
...
| 표현 | 원래 의미 | 설명 |
| :--- | :--- | :--- |
| `-n` | non-zero length | 문자열 길이가 0이 아니면 참입니다. 즉, 값이 있으면 참입니다. |
...
| 표현 | 원래 의미 | 설명 |
| :--- | :--- | :--- |
| `-z` | zero length | 문자열 길이가 0이면 참입니다. 즉, 값이 비어 있으면 참입니다. |
...
| 표현 | 원래 의미 | 설명 |
| :--- | :--- | :--- |
| `-f` | file | 해당 경로가 일반 파일이면 참입니다. |

> 주의: `pgrep -f`의 `-f`는 `full`이고, `[ -f "$LOG_FILE" ]`의 `-f`는 `file`입니다. 같은 `-f`라도 명령어마다 의미가 다릅니다.
...
| 표현 | 원래 의미 | 설명 |
| :--- | :--- | :--- |
| `-gt` | greater than | 왼쪽 숫자가 오른쪽 숫자보다 크면 참입니다. |
...
| 표현 | 의미 | 설명 |
| :--- | :--- | :--- |
| `>` | overwrite redirection | 파일을 새로 쓰거나 기존 내용을 덮어씁니다. |
| `>>` | append redirection | 파일 끝에 내용을 추가합니다. 기존 내용은 유지됩니다. |
| `2>` | stderr redirect | 표준 에러를 다른 곳으로 보냅니다. |
| `2>/dev/null` | stderr to null device | 에러 메시지를 화면에 보이지 않게 버립니다. |
| `2>&1` | stderr to stdout | 오류 출력을 정상 출력과 같은 곳으로 보냅니다. |
...
```text
정상 출력도 /tmp/test.log에 누적하고,
오류 출력도 같은 파일에 같이 기록한다.
```
...
| 표현 | 의미 | 설명 |
| :--- | :--- | :--- |
| `#!/bin/bash` | shebang | 이 파일을 Bash로 실행하라는 의미입니다. |
| `<< 'EOF'` | here document / End Of File | 여러 줄 입력을 파일로 넣을 때 사용합니다. `'EOF'`는 변수 치환을 막기 위해 사용합니다. |
| `$()` | command substitution | 괄호 안 명령어 실행 결과를 변수에 저장하거나 문자열 안에 넣습니다. |
| `${변수}` | variable expansion | 변수 값을 명확히 구분해서 사용할 때 씁니다. |
| `${APP_NAMES[@]}` | array expansion | 배열의 모든 요소를 하나씩 꺼냅니다. |
| `$(( ))` | arithmetic expansion | Bash에서 산술 계산을 합니다. |
...
| 표현 | 의미 | 설명 |
| :--- | :--- | :--- |
| `cron` | 시간 기반 작업 스케줄러 | 정해진 시간마다 명령을 자동 실행하는 리눅스 스케줄러입니다. |
| `crontab -e` | edit | 현재 사용자의 crontab을 편집합니다. |
| `crontab -l` | list | 현재 사용자의 crontab 목록을 보여줍니다. |
| `* * * * *` | 매분 실행 | 분, 시, 일, 월, 요일이 모두 `*`이므로 매분 실행한다는 뜻입니다. |

cron 표현의 각 위치는 다음과 같습니다.
...
| 표현 | 뜻 | 한 줄 설명 |
| :--- | :--- | :--- |
| `pgrep -f` | full | 전체 명령줄에서 프로세스 검색 |
| `ss -t` | TCP | TCP 소켓 확인 |
| `ss -u` | UDP | UDP 소켓 확인 |
| `ss -l` | listening | LISTEN 상태 확인 |
| `ss -n` | numeric | 숫자 포트로 표시 |
| `ss -p` | process | 포트를 쓰는 프로세스 표시 |
| `top -b` | batch | 스크립트 처리용 출력 |
| `top -n1` | number 1 | 한 번만 실행 |
| `cut -d` | delimiter | 구분자 지정 |
| `cut -f` | field | 가져올 필드 지정 |
| `stat -c` | format | 출력 형식 지정 |
| `stat %s` | size | 파일 크기 출력 |
| `[ -n ]` | non-zero length | 문자열이 비어 있지 않으면 참 |
| `[ -z ]` | zero length | 문자열이 비어 있으면 참 |
| `[ -f ]` | file | 일반 파일이면 참 |
| `[ -gt ]` | greater than | 숫자가 더 크면 참 |
| `>` | overwrite | 덮어쓰기 |
| `>>` | append | 뒤에 누적 |
| `2>` | stderr redirect | 에러 출력 리다이렉션 |
| `2>&1` | stderr to stdout | 에러를 정상 출력과 같은 곳으로 보냄 |
| `<< EOF` | End Of File | 여러 줄 입력의 끝 표시 |
| `$()` | command substitution | 명령 결과를 변수에 저장 |
| `$(( ))` | arithmetic expansion | 산술 계산 |
| `crontab -e` | edit | cron 편집 |
| `crontab -l` | list | cron 목록 확인 |
...
| 표현 | 의미 |
| :--- | :--- |
| `pgrep -f` | full command line, 전체 명령줄 검색 |
| `[ -f "$LOG_FILE" ]` | file, 일반 파일인지 확인 |
...
| :--- | :--- |
| `ss -n` | numeric, 숫자로 출력 |
| `[ -n "$PID" ]` | non-zero length, 문자열이 비어 있지 않음 |
| `top -n1` | number of iterations, 1번 실행 |
...
`monitor.log`는 누적 기록이 중요하므로 `>`가 아니라 `>>`를 사용합니다.
